...
namespace Airport_Airplane_management_system.View.Forms.UserPages
{
    public partial class UserAccount : UserControl, IUserAccountView
    {
        ...
        public void Initialize(INavigationService navigation)
        {
            ...
            _presenter = new Presenter.UserAccountPresenter(this, _service);
        ...
        public void SetHeader(string username, string email, DateTime createdAt, DateTime? lastLoginAt)
        {
            ...
            lblCreatedAtValue.Text = createdAt == DateTime.MinValue
                ? "-"
                : createdAt.ToString("yyyy-MM-dd HH:mm");

            if (lastLoginAt.HasValue)
            {
                var db = lastLoginAt.Value;

                // treat as UTC and convert to local
                var asUtcToLocal = DateTime.SpecifyKind(db, DateTimeKind.Utc).ToLocalTime();

                // treat as local (no conversion)
                var asLocal = DateTime.SpecifyKind(db, DateTimeKind.Local);

                // pick the one closer to now (auto-detect)
                var now = DateTime.Now;
                var pickUtc = Math.Abs((now - asUtcToLocal).TotalHours) < Math.Abs((now - asLocal).TotalHours);

                lblLastLoginValue.Text = (pickUtc ? asUtcToLocal : asLocal).ToString("yyyy-MM-dd HH:mm");
            }
            else
            {
