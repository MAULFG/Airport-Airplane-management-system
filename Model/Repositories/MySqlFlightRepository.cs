...
namespace Airport_Airplane_management_system.Model.Repositories
{
    public class MySqlFlightRepository : IFlightRepository
    {
        ...
        public List<FlightSeats> GetSeatsForFlight(int flightId)
        {
            ...
            const string query = @"
SELECT id, flight_id, plane_seat_index, seat_number, class_type, is_booked, passenger_id, seat_price
FROM flight_seats
WHERE flight_id = @fid
ORDER BY plane_seat_index;";
            ...
            while (reader.Read())
            {
                ...
                // ✅ passenger_id (not user_id)
                int? passengerId = reader.IsDBNull(reader.GetOrdinal("passenger_id"))
                    ? (int?)null
                    : reader.GetInt32("passenger_id");

                decimal price = reader.GetDecimal("seat_price");

                seats.Add(new FlightSeats(seatId, flightId, planeSeatIndex, seatNum, classType, isBooked, passengerId, price));
            }
